const Play ={
	...
	Login: class extends Packet{
		...
		constructor(protocol=282,body=new Types.LoginBody()){
			super();
	...
	StartGame: class extends Packet{
		...
		encodeBody(reset){
			...
			this.writeVaruint(this.runtimeIdTable.length);
			for(var dhc5d5a1 in this.runtimeIdTable){
				this.writeVarulong(this.runtimeIdTable[dhc5d5a1]);
			}
			var dhc5dxab=this.encodeString(this.multiplayerCorrelationId);
			this.writeVaruint(dhc5dxab.length);
			this.writeBytes(dhc5dxab);
			return new Uint8Array(this._buffer);
		}
		...
		decodeBody(_buffer){
			...
			this.runtimeIdTable=[];
			for(var dhc5d5a1=0;dhc5d5a1<aramcvdl;dhc5d5a1++){
				this.runtimeIdTable[dhc5d5a1]=this.readVarulong();
			}
			traceDecode('runtimeIdTable');
			var dhc5dxab=this.readVaruint();
			this.multiplayerCorrelationId=this.decodeString(this.readBytes(dhc5dxab));
			traceDecode('multiplayerCorrelationId');
			return this;
		}
	...
	AddPlayer: class extends Packet{
		...
		constructor(uuid=new Types.McpeUuid(),username="",entityId=0,runtimeId=0,position={x:0,y:0,z:0},motion={x:0,y:0,z:0},pitch=.0,headYaw=.0,yaw=.0,heldItem=new Types.Slot(),metadata=new Metadata(),unknown11=0,unknown12=0,unknown13=0,unknown14=0,unknown15=0,unknown16=0,links=[],deviceId=""){
			super();
			this.uuid = uuid;
		...
		encodeBody(reset){
			...
			var dhc5zzyv=this.encodeString(this.deviceId);
			this.writeVaruint(dhc5zzyv.length);
			this.writeBytes(dhc5zzyv);
			return new Uint8Array(this._buffer);
		}
		...
		decodeBody(_buffer){
			...
			traceDecode('links');
			var dhc5zzyv=this.readVaruint();
			this.deviceId=this.decodeString(this.readBytes(dhc5zzyv));
			traceDecode('deviceId');
			return this;
		}
	...
	AddEntity: class extends Packet{
		...
		constructor(entityId=0,runtimeId=0,type=0,position={x:0,y:0,z:0},motion={x:0,y:0,z:0},pitch=.0,yaw=.0,headYaw=.0,attributes=[],metadata=new Metadata(),links=[]){
			super();
			this.entityId = entityId;
			this.runtimeId = runtimeId;
			this.type = type;
	...
	MoveEntityAbsolute: class extends Packet{
		...
		constructor(entityId=0,flags=0,position={x:0,y:0,z:0},rotation={x:0,y:0,z:0}){
			super();
			this.entityId = entityId;
			this.flags = flags;
			this.position = position;
		...
		encodeBody(reset){
			...
			this.writeLittleEndianFloat(this.position.y);
			this.writeLittleEndianFloat(this.position.z);
			this.writeByte(this.rotation.x);
			this.writeByte(this.rotation.y);
			this.writeByte(this.rotation.z);
			return new Uint8Array(this._buffer);
		}
		...
		decodeBody(_buffer){
			...
			traceDecode('position');
			this.rotation={};
			this.rotation.x=this.readByte();
			this.rotation.y=this.readByte();
			this.rotation.z=this.readByte();
	...
	MoveEntityDelta: class extends Packet{

		static get ID(){return 111;};
		...
		constructor(flags=0,xDiff=0,yDiff=0,zDiff=0,xRot=0,yRot=0,zRot=0){
			super();
			this.flags = flags;
			this.xDiff = xDiff;
			this.yDiff = yDiff;
		...
		encodeBody(reset){
			...
			this.writeByte(this.flags);
			if((flags&1)!=0){
				this.writeVarint(this.xDiff);
			}
			if((flags&2)!=0){
				this.writeVarint(this.yDiff);
			}
			if((flags&4)!=0){
				this.writeVarint(this.zDiff);
			}
			if((flags&8)!=0){
				this.writeByte(this.xRot);
			}
			if((flags&16)!=0){
				this.writeByte(this.yRot);
			}
			if((flags&32)!=0){
				this.writeByte(this.zRot);
			}
			return new Uint8Array(this._buffer);
		}
		...
		decodeBody(_buffer){
			...
			traceDecode('flags');
			if((flags&1)!=0){
				this.xDiff=this.readVarint();
				traceDecode('xDiff');
			}
			if((flags&2)!=0){
				this.yDiff=this.readVarint();
				traceDecode('yDiff');
			}
			if((flags&4)!=0){
				this.zDiff=this.readVarint();
				traceDecode('zDiff');
			}
			if((flags&8)!=0){
				this.xRot=this.readByte();
				traceDecode('xRot');
			}
			if((flags&16)!=0){
				this.yRot=this.readByte();
				traceDecode('yRot');
			}
			if((flags&32)!=0){
				this.zRot=this.readByte();
				traceDecode('zRot');
			}
			return this;
		}
	...
	SetScoreboardIdentity: class extends Packet{

		static get ID(){return 112;};
		...
		constructor(type=0,registerEntries=[],clearEntries=[]){
			super();
			this.type = type;
		...
		encodeBody(reset){
			...
			this.writeByte(this.type);
			if(type==0){
				this.writeVaruint(this.registerEntries.length);
				for(var dhc5zdcr in this.registerEntries){
					this.writeBytes(this.registerEntries[dhc5zdcr].encodeBody(true));
				}
			}
			if(type==1){
				this.writeVaruint(this.clearEntries.length);
				for(var dhc5bvcv in this.clearEntries){
					this.writeVarlong(this.clearEntries[dhc5bvcv]);
				}
			}
			return new Uint8Array(this._buffer);
		}
		...
		decodeBody(_buffer){
			...
			traceDecode('type');
			if(type==0){
				var aramcvan=this.readVaruint();
				this.registerEntries=[];
				for(var dhc5zdcr=0;dhc5zdcr<aramcvan;dhc5zdcr++){
					this.registerEntries[dhc5zdcr]=new Types.RegisterEntry().decodeBody(this._buffer);
					this._buffer=this.registerEntries[dhc5zdcr]._buffer;
				}
				traceDecode('registerEntries');
			}
			if(type==1){
				var aramyxyj=this.readVaruint();
				this.clearEntries=[];
				for(var dhc5bvcv=0;dhc5bvcv<aramyxyj;dhc5bvcv++){
					this.clearEntries[dhc5bvcv]=this.readVarlong();
				}
				traceDecode('clearEntries');
			}
			return this;
		}
	...
	SetLocalPlayerAsInitialized: class extends Packet{

		static get ID(){return 113;};
		...
		encodeBody(reset){
			...
			this.writeVarulong(this.entityRuntimeId);
			return new Uint8Array(this._buffer);
		}
		...
		decodeBody(_buffer){
			this._buffer=Array.from(_buffer);
			initDecode(this);
			this.entityRuntimeId=this.readVarulong();
			traceDecode('entityRuntimeId');
			return this;
		}
	...
	UpdateSoftEnum: class extends Packet{

		static get ID(){return 114;};
		...
		constructor(name="",values=[],type=0){
			super();
			this.name = name;
			this.values = values;
			this.type = type;
		...
		encodeBody(reset){
			...
			this.writeVaruint(dhc5y1.length);
			this.writeBytes(dhc5y1);
			this.writeVaruint(this.values.length);
			for(var dhc5yxzm in this.values){
				var dhc5yxzn=this.encodeString(this.values[dhc5yxzm]);
				this.writeVaruint(dhc5yxzn.length);
				this.writeBytes(dhc5yxzn);
			}
			this.writeByte(this.type);
			return new Uint8Array(this._buffer);
		}
		...
		decodeBody(_buffer){
			...
			this.values=[];
			for(var dhc5yxzm=0;dhc5yxzm<aramdfdv;dhc5yxzm++){
				var dhc5yxzn=this.readVaruint();
				this.values[dhc5yxzm]=this.decodeString(this.readBytes(dhc5yxzn));
			}
			traceDecode('values');
			this.type=this.readByte();
			traceDecode('type');
	...
	NetworkStackLatency: class extends Packet{

		static get ID(){return 115;};
		...
		encodeBody(reset){
			...
			this.writeLittleEndianLong(this.timestamp);
			return new Uint8Array(this._buffer);
		}
		...
		decodeBody(_buffer){
			this._buffer=Array.from(_buffer);
			initDecode(this);
			this.timestamp=this.readLittleEndianLong();
			traceDecode('timestamp');
			return this;
		}
