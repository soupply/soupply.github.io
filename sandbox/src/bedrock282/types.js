const Types ={
	...
	Varulong: class extends Buffer{

		constructor(name="",data=0){
			super();
			this.name = name;
			this.data = data;
		}
		...
		encodeBody(reset){
			...
			this.writeVaruint(dhc5y1.length);
			this.writeBytes(dhc5y1);
			this.writeLittleEndianShort(this.data);
			return new Uint8Array(this._buffer);
		}
		...
		decodeBody(_buffer){
			...
			this.name=this.decodeString(this.readBytes(dhc5y1));
			traceDecode('name');
			this.data=this.readLittleEndianShort();
			traceDecode('data');
			return this;
		}
	...
	CommandOriginData: class extends Buffer{

		constructor(type=0,uuid=new Uint8Array(16),requestId="",unknown3=0){
			super();
			this.type = type;
			this.uuid = uuid;
	...
	CommandMessage: class extends Buffer{

		constructor(isInternal=false,messageId="",parameters=[]){
			super();
		...
		encodeBody(reset){
			...
			var dhc5znyd=this.encodeString(this.messageId);
			this.writeVaruint(dhc5znyd.length);
			this.writeBytes(dhc5znyd);
			this.writeVaruint(this.parameters.length);
			for(var dhc5yjbv in this.parameters){
				var dhc5yjbv=this.encodeString(this.parameters[dhc5yjbv]);
				this.writeVaruint(dhc5yjbv.length);
				this.writeBytes(dhc5yjbv);
			}
			return new Uint8Array(this._buffer);
		}
		...
		decodeBody(_buffer){
			...
			traceDecode('isInternal');
			var dhc5znyd=this.readVaruint();
			this.messageId=this.decodeString(this.readBytes(dhc5znyd));
	...
	Score: class extends Buffer{

		constructor(uuid=new Uint8Array(16),objectiveName="",score=0){
			super();
			this.uuid = uuid;
		...
		encodeBody(reset){
			...
			this.writeLittleEndianInt(this.score);
			return new Uint8Array(this._buffer);
		}
		...
		decodeBody(_buffer){
			...
			this.score=this.readLittleEndianInt();
			traceDecode('score');
			return this;
		}
	...
	RegisterEntry: class extends Buffer{

		constructor(scoreboardId=0,uuid=new Uint8Array(16)){
			super();
		...
		encodeBody(reset){
			...
			this.writeVarlong(this.scoreboardId);
			this.writeBytes(this.uuid);
			return new Uint8Array(this._buffer);
		}
		...
		decodeBody(_buffer){
			this._buffer=Array.from(_buffer);
			initDecode(this);
			this.scoreboardId=this.readVarlong();
			traceDecode('scoreboardId');
			this.uuid=this.readBytes(16);
			traceDecode('uuid');
			return this;
		}
