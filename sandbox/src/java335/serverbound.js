 ...
 */
// module soupply.java335.serverbound;
...
const Serverbound = {
	...
	CraftingGrid: class extends Buffer {
		...
		constructor(window=0, action=0, returnEntry=null, prepareEntry=null) {
			super();
			this.window = window;
			this.action = action;
		...
		encode() {
			...
			this.writeBigEndianShort(this.return_entry.length);
			for(var dhc5zrc5 in this.return_entry) {
				this.writeBytes(this.return_entry[dhc5zrc5].encode());
			}
			this.writeBigEndianShort(this.prepare_entry.length);
			for(var dhc5cvyj in this.prepare_entry) {
				this.writeBytes(this.prepare_entry[dhc5cvyj].encode());
			}
			return new Uint8Array(this._buffer);
		}
		...
		decode(_buffer) {
			...
			for(var dhc5zrc5=0;dhc5zrc5<aramcvdj;dhc5zrc5++) {
				this.return_entry[dhc5zrc5]=Types.Entry.fromBuffer(this._buffer);
				this._buffer=this.return_entry[dhc5zrc5]._buffer;
			}
			var aramcjcf=this.readBigEndianShort();
			this.prepare_entry=[];
			for(var dhc5cvyj=0;dhc5cvyj<aramcjcf;dhc5cvyj++) {
				this.prepare_entry[dhc5cvyj]=Types.Entry.fromBuffer(this._buffer);
				this._buffer=this.prepare_entry[dhc5cvyj]._buffer;
			}
			return this;
		}
		...
		/** @param {(Uint8Array|Array)} buffer */
		static fromBuffer(buffer) {
			return new Serverbound.CraftingGrid().decode(buffer);
		}

		/** @return {string} */
		toString() {
			return "CraftingGrid(window: " + this.window + ", action: " + this.action + ", return_entry: " + this.return_entry + ", prepare_entry: " + this.prepare_entry + ")";
		}
	...
	CraftingBookData: class extends Buffer {
		...
		decode(_buffer) {
			...
			switch(this.type) {
				case 1:
					...
					break;
				case 2:
					this.book_opened=this.readBool();
		...
		/** @param {(Uint8Array|Array)} buffer */
		static fromBuffer(buffer) {
			return new Serverbound.CraftingBookData().decode(buffer);
		}
	...
	AdvencementTab: class extends Buffer {
		...
		/** @param {(Uint8Array|Array)} buffer */
		static fromBuffer(buffer) {
			return new Serverbound.AdvencementTab().decode(buffer);
		}
