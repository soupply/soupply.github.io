const Serverbound ={
	...
	CraftingGrid: class extends Packet{
		...
		encodeBody(reset){
			...
			this.writeBigEndianShort(this.returnEntry.length);
			for(var dhc5zrc5 in this.returnEntry){
				this.writeBytes(this.returnEntry[dhc5zrc5].encodeBody(true));
			}
			this.writeBigEndianShort(this.prepareEntry.length);
			for(var dhc5cvyj in this.prepareEntry){
		...
		decodeBody(_buffer){
			...
			for(var dhc5cvyj=0;dhc5cvyj<aramcjcf;dhc5cvyj++){
				...
				this._buffer=this.prepareEntry[dhc5cvyj]._buffer;
			}
			traceDecode('prepareEntry');
			return this;
		}
